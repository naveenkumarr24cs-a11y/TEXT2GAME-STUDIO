...
import { Bot, BrainCircuit, ChevronRight, Loader2, Send, Sparkles, Terminal, User, Zap, Lightbulb, Cpu, Bolt, Brain, RefreshCw, Workflow, Plus, Paperclip, X, Image as ImageIcon, Film, Mic, MicOff } from 'lucide-react';
      ...
      recognition.onresult = (event: any) => {
        ...
        if (finalTranscript) {
          setInput(prev => {
            ...
            return trimmedPrev === '' ? finalTranscript : trimmedPrev + ' ' + finalTranscript;
          });
        }
      };
      ...
      >
        ...
        <div className="relative flex items-end gap-2 bg-slate-50 border border-slate-200 rounded-2xl md:rounded-[1.75rem] p-2 focus-within:border-indigo-600 focus-within:bg-white transition-all shadow-inner">
          <button
            ...
            disabled={isProcessing}
            className="p-3 md:p-4 text-slate-400 hover:text-indigo-600 transition-colors shrink-0"
          >
          ...
          <button
            ...
            disabled={isProcessing}
            className={`p-3 md:p-4 transition-colors relative shrink-0 ${isListening ? 'text-red-500 animate-pulse' : 'text-slate-400 hover:text-indigo-600'}`}
